...
class FW_Option_Type_ThzSection_Builder_Item_Columns extends FW_Option_Type_ThzSection_Builder_Item {
	...
	public function get_thumbnails() {
		return array(
			array(
				'html' =>
					'<div class="item-type-icon-title" data-hover-tip="' . esc_html__( 'Add a widget area', 'creatus' ) . '">' .
					'<div class="item-type-icon"><img src="' . esc_attr( $this->get_uri( '/static/images/icon.png' ) ) . '" /></div>' .
					'<div class="item-type-title">' . esc_html__( 'Widget area', 'creatus' ) . '</div>' .
					'</div>'
			)
		);
	}
	...
	public function enqueue_static() {
		...
		wp_localize_script(
			...
			array(
				'l10n'     => array(
					'item_title'      => esc_html__( 'Widget options', 'creatus' ),
...
FW_Option_Type_Builder::register_item_type( 'FW_Option_Type_ThzSection_Builder_Item_Columns' );
