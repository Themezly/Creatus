 ...
 *  Under MIT License
 *	Modified by themezly look for themezly in uncompressed version
	...
	Plugin.prototype = {
		...
		renderIconContainer: function () {
			...
			if (!this.settings.emptyIcon && (!this.element.val() || $.inArray(this.element.val(), this.settings.source) === -1)) {
				...
				//this.setSelectedIcon(''); // Removed by themezly, it is empty default value on category change
				...
				this.setSelectedIcon(passDefaultIcon,true);// changed by themezly to detect first load and dont trigger change
			}
		 ...
		 */
		setSelectedIcon: function (theIcon,firstload) {
			...
			// changed by themezly, Dont trigger change on first load
			if(!firstload){
