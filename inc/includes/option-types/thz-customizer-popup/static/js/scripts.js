(function ($, _, fwEvents, window) {
	var ThzCustomizerPopup = function () {
		var $this = $(this),
			...
			utils = {
				...
				editItem: function (item, values) {
					...
					if (val != values) {
						$this.trigger('fw:option-type:thz-customizer-popup:change');
		...
		utils.modal.on({
			'change:values': function (modal, values) {
				...
				fwEvents.trigger('fw:option-type:thz-customizer-popup:change', {
					element: $this,
					values: values
				});
			},
			'open': function () {
				$this.trigger('fw:option-type:thz-customizer-popup:open');

				if (data['custom-events']['open']) {
					fwEvents.trigger('fw:option-type:thz-customizer-popup:custom:' + data['custom-events']['open'], {
						element: $this,
						modal: utils.modal
					});
				}
			...
			'close': function () {
				$this.trigger('fw:option-type:thz-customizer-popup:close');

				if (data['custom-events']['close']) {
					fwEvents.trigger('fw:option-type:thz-customizer-popup:custom:' + data['custom-events']['close'], {
						element: $this,
						modal: utils.modal
					});
				}
			...
			'render': function () {
				$this.trigger('fw:option-type:thz-customizer-popup:render');

				if (data['custom-events']['render']) {
					fwEvents.trigger('fw:option-type:thz-customizer-popup:custom:' + data['custom-events']['render'], {
						element: $this,
						modal: utils.modal
					});
				}
	...
	fwEvents.on('fw:options:init', function (data) {
		data.$elements
			.find('.fw-option-type-thz-customizer-popup:not(.fw-option-initialized)').each(ThzCustomizerPopup)
			.addClass('fw-option-initialized');
	});

	fw.options.register('thz-customizer-popup', {
