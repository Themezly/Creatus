(function($,_) {
	...
	var init = function($calendarWrapper){
		...
		var options = {
				...
				view: template,
				tmpl_path: templatePath, //+ '/inc/thzframework/extensions/shortcodes/shortcodes/calendar/views/',
				first_day: firstDay,
				//time_start:         '06:00',
				//time_end:           '22:00',
				...
				onAfterViewLoad: function(view) {
					$calendarWrapper.find('.thz-calendar-title').text(this.getTitle());
					$calendarWrapper.find('.thz-calendar-header button').removeClass('active');
					...
					$calendarWrapper.find('.hide-calendar-section').removeClass('hide-calendar-section');
			...
			calendar = $calendarWrapper.find('.thz-calendar').calendar(options);
			$calendarWrapper.data('thz-calendar.calendar', calendar);

		//navigation buttons
		{
			$calendarWrapper.find('.thz-calendar-header button[data-calendar-nav]').each(function() {
				var $this = $(this);
				$this.click(function() {
	...
	$(document).ready(function(){
		$('.thz-calendar-wrapper:not(fw-initialized)').each(function(){
			init($(this));
